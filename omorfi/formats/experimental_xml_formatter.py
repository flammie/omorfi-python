...
def format_multichars_lexc_xml():
    ...
    multichars += """
    <!-- Following specials exist in all versions of omorfi -->
...
def format_root_lexicon_xml():
    """Format root lexicon in xml."""
    root = "  <LEXICON id=\"Root\">\n"
    root += """<!-- ... -->
    <e><a/><i/><cont lexica=\"NOUNS ADJECTIVES VERBS NUMERALS DIGITSS ACRONYMS
    PRONOUNS PARTICLES PUNCTUATIONS FIFTYONE\"/></e>
"""
    root += "    <e><a><s mcs=\"B_right\"/></a><i>-</i><cont lexica=\"NOUN"
    root += "ADJECTIVE SUFFIX\"/></e>\n"
    root += "    <e><a><s mcs=\"VERSION\"/></a><i/>"
    root += "<cont lexica=\"_end\"/></e>\n"
    root += "  </LEXICON>\n"
    return root


def format_lexc_xml(wordmap):
    """Format word in lexc xml."""
